...
Alguses tee "pip install requests beautifulsoup4 playwright" terminalis, kuna need pole standard teegid. Lisaks "playwright install"
"""
...
def brauser():
    ...
    page.set_default_timeout(25000) # ms
    
    # Sain siit leheküljelt baas versiooni ja muutsin seda vastavalt vajdusele
    # https://www.zenrows.com/blog/blocking-resources-in-playwright#prerequisites
    page.route("**/*", 
...
def värskenda_context_küpsised(vana_context):
    """
...
def toote_scraper(page, jär, kategooria_nimi):
    ...
    toote_protsent_muster = re.compile(r"(\d+(?:\s*[.,]\s*\d+)?)\s*%", re.IGNORECASE)
    
    toote_maht_muster = re.compile(r"""
                                    (\d+(?:\s*[.,]\s*\d+)?)  # ühiku väärtus nt 500
                                    \s*
                                    (ml|cl|l)                # ühik
                                    """ 
    ...
    hind_muster = re.compile(r"(\d+(?:\s*[.,]\s*\d+)?)\s*€", re.IGNORECASE)
    
    viimati_uuendatud_muster = re.compile(r"""
                                            (?:(\d+)[\s\u00A0\u202F]*)?     # väärtus
                                            ([\wäöüõ]+)                     # ühik
    ...
    for el in jär:
        ...
        if mitmes_toode != 0 and mitmes_toode % 100 == 0:
            page = värskenda_context_küpsised(page.context)
            print("Scraper värskendatud")
        

        toode_el = el[0].strip()
        toode = toode_el.lower()
        ...
        toote_protsent = float(protsent_tekst.group(1).replace(',', '.').replace(" ", ""))
        ...
        if not maht_tekst:
            ...
            continue
        toote_maht = float(maht_tekst.group(1).replace(',', '.').replace(" ", ""))
        ...
        for pood in poed:
            ...
            if hind_el:
                ...
                hind = float(hind_muster.search(hind_tekst).group(1).replace(',', '.').replace(" ", ""))
            else:
                hind = None
            ...
            if viimati_uuendatud_el:
                ...
                if viimati_uuendatud_tekst:
                    ...
                    ühik_aeg = viimati_uuendatud_tekst.group(2)
                    sekundi_väärtus = ühikud.get(ühik_aeg)
...
def main():
    ...
    for kategooria_nimi, leht in kategooriad:
        ...
        page = toote_scraper(page, tooted, kategooria_nimi)
